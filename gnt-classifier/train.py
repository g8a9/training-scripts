...
class ModelArguments:
    ...
    use_weighted_loss: bool = field(default=False)
...
def main():
    ...
    train_df = pd.read_csv(data_args.train_file, sep="\t")
    train_df[data_args.label_column_name] = train_df[data_args.label_column_name].apply(lambda x: "neutral" if x == 0 else "gendered")
    print("Label distribution training", train_df[data_args.label_column_name].value_counts(normalize=True))
    validation_df = pd.read_csv(data_args.validation_file, sep="\t")
    validation_df[data_args.label_column_name] = validation_df[data_args.label_column_name].apply(lambda x: "neutral" if x == 0 else "gendered")
    print("Label distribution validation", validation_df[data_args.label_column_name].value_counts(normalize=True))
    raw_datasets = DatasetDict({
        "train": Dataset.from_pandas(train_df),
        "validation": Dataset.from_pandas(validation_df)
    })

    pos_weights = len(train_df) / (2 * train_df[data_args.label_column_name].value_counts()[1])
    neg_weights = len(train_df) / (2 * train_df[data_args.label_column_name].value_counts()[0])
    ...
    if not hasattr(tokenizer, "pad_token_id"):
        logger.info("Setting the pad token to the tokenizer")
    ...
    if training_args.do_train and not is_regression:  # classification, training
        ...
        label_to_id = {"neutral": 0, "gendered": 1}
        # update config with label info8
    ...
    with training_args.main_process_first(desc="dataset map pre-processing"):
        raw_datasets = raw_datasets.map(
            ...
            num_proc=training_args.dataloader_num_workers * 2
        )
    ...
    def compute_metrics_old(p: EvalPrediction):
    ...
    if data_args.pad_to_max_length:
        ...
        logger.info("Dynamic padding enabled!")
    ...
    trainer_cls = WeightedCELossTrainer if model_args.use_weighted_loss else Trainer
